...
def extract_utils_folder(temp_folder, target_folder, repo_temp):
    repo_folder = os.path.join(temp_folder, repo_temp)
    ...
    if config and config.get('use_Git', False):
        ...
        if repo_url:
            try:
                ...
                extract_functions_folder(temp_folder, functions_target, config['repo_temp'])
                extract_utils_folder(temp_folder, utils_target, config['repo_temp'])
            finally:
